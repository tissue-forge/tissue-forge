...
# new simulator
tf.init(dim=dim, cutoff=cutoff, dt=0.001, max_distance=0.2, threads=8, cells=[5, 5, 5])
...
pts = [p * (Big.radius + Small.radius) + tf.Universe.center for p in tf.random_points(tf.PointsType.Sphere.value, count)]
Small.factory(positions=pts)
