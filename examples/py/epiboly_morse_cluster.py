...
tf.init(dim=dim, cutoff=3, dt=0.001, cells=[6, 6, 6])
...
tf.event.on_particletime(ptype=C, invoke_method=split, period=1, selector="largest")
...
c = C(position=yolk.position + [0, 0, yolk.radius + C.radius - 10])
...
rforce = tf.Force.random(mean=0, std=500, duration=0.0001)

tf.bind.force(rforce, B)
tf.bind.types(pb, B, B, bound=True)
tf.bind.types(pub, B, B, bound=False)
