...
"""
This example demonstrates flux sub-stepping.

Flux calculations can be performed in sub-steps to handle numerical instabilities caused by fast diffusion.
Increase in computational cost is proportional to the number of sub-steps.

See the ``init`` keyword argument "flux_steps". When set to 1 (i.e., no sub-stepping), diffusion will be too fast.
"""
...
tf.Fluxes.flux(carrier_type, carrier_type, "S1", 1.0)
tf.Fluxes.flux(carrier_type, sink_type, "S1", 500.0)
