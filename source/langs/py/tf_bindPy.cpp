...
namespace TissueForge::py {
        ...
        uint32_t flags) 
    {
        ...
        std::vector<BondHandle*> *_result = NULL;
    ...
    std::vector<BondHandle*> _spherePy(
        ...
        std::pair<ParticleList*, std::vector<BondHandle*>*> *result = NULL;
        bind::sphere(potential, n, center,radius, phi, type, &result);
        if(result) { 
            particles = result->first;
            std::vector<BondHandle*> *_bonds = result->second;
            bonds = *_bonds;
            delete _bonds;
            delete result;
        }
